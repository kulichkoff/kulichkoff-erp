...
function createEntitiesIndex() {
    console.log('Creating entity-index.ts for api-bills-db app');
    const src = `${path.dirname(__dirname)}/apps/api/bills-db/src`;
    if (!fs.existsSync(src)) {
        console.log(`App api-bills-db cannot be found. Path not exist: ${src}`);
        process.exit(1);
    }
